...
class SepPspDSDataConfig(DataConfig):
    ...
    # sep_psp_index_path is a path, so it must join the base class's list to get the same
    # relative-to-the-config-file resolution. Extend this whenever you add a path field.
    PATH_FIELDS: ClassVar[tuple[str, ...]] = DataConfig.PATH_FIELDS + ("sep_psp_index_path",)


# The app's entry point. Identical to load_config() except that the data: section is
# parsed into SepPspDSDataConfig, so the four keys above are recognized instead of rejected.
