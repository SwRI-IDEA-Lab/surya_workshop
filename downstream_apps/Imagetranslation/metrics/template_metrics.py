...
class ImageTranslationMetrics:
    def __init__(self, mode: str):
        """
        Initialize ImageTranslationMetrics class.
    ...
    def train_metrics(
        ...
        """
        Calculate evaluation metrics for training.
        IMPORTANT: These metrics are only for reporting and do not
        ...
        self._ensure_device(preds)
        output_metrics["mse"] = self._mse(preds, target)
        output_weights.append(1)
        output_metrics["mae"] = self._mae(preds, target)
        output_weights.append(1)
        #output_metrics["rrse"] = self._rrse(preds, target)
        #output_weights.append(1)
    ...
    def __call__(
        ...
        dict
            Dictionary with all metrics. Metrics aggregate over the batch. So the
            dictionary takes the shape [str, torch.Tensor] with the tensors having
            shape [].
        list
            List of weights for each calculated metric to enable giving a different
            weight to each loss term.
