...
class FilamentLightningModule(pl.LightningModule):
    ...
    def training_step(self, batch: Dict[str, Any], batch_idx: int) -> torch.Tensor:
        ...
        """
        #target = batch["mask"]#.unsqueeze(1).float()
        target = batch["mask"]#.unsqueeze(1).float()
        output = self(batch)
        training_losses, training_loss_weights = self.training_loss(output.reshape(output.shape[0], -1), target.reshape(target.shape[0], -1))
        ...
        training_evaluation_metrics, training_evaluation_weights = self.training_evaluation(output.reshape(output.shape[0], -1), target.reshape(output.shape[0], -1))
    ...
    def validation_step(self, batch: Dict[str, Any], batch_idx: int) -> None:
        ...
        output = self(batch)
        val_losses, val_loss_weights = self.training_loss(output.reshape(output.shape[0], -1), target.reshape(output.shape[0], -1))
        ...
        # Log evaluation metrics (optional).
        val_evaluation_metrics, val_evaluation_weights = self.validation_evaluation(output.reshape(output.shape[0], -1), target.reshape(output.shape[0], -1))
