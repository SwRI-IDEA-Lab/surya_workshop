...
def dice_loss(preds: torch.Tensor, target: torch.Tensor, smooth: float = 1.0) -> torch.Tensor:
    """
    Dice loss for binary segmentation. Handles extreme class imbalance better than BCE.
    ...
    # Flatten to compute over all pixels
    probs_flat = probs.view(-1)
    target_flat = target.view(-1)
    
    intersection = (probs_flat * target_flat).sum()
    return 1 - (2. * intersection + smooth) / (probs_flat.sum() + target_flat.sum() + smooth)


class RibbonSegmentationMetrics:
    def __init__(self, mode: str, threshold: float = 0.5, pos_weight: float = 1000.0):
        """
        ...
        Args:
            ...
            threshold (float): Threshold for converting predictions to binary masks.
                              Default is 0.5.
            pos_weight (float): Weight for positive class in BCE loss to handle
                               class imbalance. Default is 1000.0 (suitable for
                               very sparse masks like flare ribbons).
        """
        self.mode = mode
    ...
    def train_loss(
        ...
        output_metrics["bce"] = torch.nn.functional.binary_cross_entropy_with_logits(
            preds, target, pos_weight=pos_weight_tensor
        )
        output_weights.append(0.5)

        # Dice loss - directly optimizes for overlap, handles imbalance well
        output_metrics["dice"] = dice_loss(preds, target)
        output_weights.append(0.5)
    ...
    def val_metrics(
        ...
        # Convert target to float for BCE
        target_float = target.float()
        
        # Weighted BCE loss
        pos_weight_tensor = torch.tensor([self.pos_weight], device=preds.device)
        output_metrics["bce"] = torch.nn.functional.binary_cross_entropy_with_logits(
            preds, target_float, pos_weight=pos_weight_tensor
        )
        output_weights.append(1)

        # Dice loss
        output_metrics["dice"] = dice_loss(preds, target_float)
        output_weights.append(1)
