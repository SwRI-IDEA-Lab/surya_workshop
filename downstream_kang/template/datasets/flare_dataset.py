...
class FlareForecastingDataset(HelioNetCDFDatasetAWS):
    """
    ...
    def __init__(
        ...
        if ds_flare_index_path is not None:
            ...
            raise ValueError(
                "ds_flare_index_path must be provided for FlareDSDataset")
        ...
        if "label_max" not in self.ds_index.columns:
            raise ValueError("CSV must have a 'label_max' column")

        unique_labels = self.ds_index["label_max"].unique()
        if not set(unique_labels).issubset({0, 1}):
            raise ValueError(
                f"label_max should only contain 0 and 1, but found: {unique_labels}")

        self.ds_index["label_max"] = self.ds_index["label_max"].astype(
            np.float32)
        ...
        self.df_valid_indices["index_delta"] = np.abs(
            ...
            self.df_valid_indices["ds_index"]
        )
        ...
        if ds_time_tolerance is not None:
            self.df_valid_indices = self.df_valid_indices.loc[
                self.df_valid_indices["index_delta"] <= pd.Timedelta(
                    ds_time_tolerance),
                :,
            ]
            if len(self.df_valid_indices) == 0:
                raise ValueError(
                    "No intersection between Surya and DS indices")
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        base_dictionary["forecast"] = self.df_valid_indices.iloc[idx]["label_max"]
        base_dictionary["ds_index"] = self.df_valid_indices["ds_index"].iloc[idx].isoformat()
